...
<body>
	...
	<h1><?=$lottery->name ?></h1>
	<form id='start' style="border-style: solid;width:350px;display:none">
	<?=form_hidden('ajax_do',1) ?>
	<?=form_hidden('lottery_id',$lottery->id) ?>
	</form>
    ...
    var KinerLottery = new KinerLottery({
        ...
        clickCallback: function() {
			var $this = this;
	        var angle = 0;
			$.post('/service/lottery/action/lists_lottery',$('#start').serializeArray(),function(ret){
				if(ret.code==1){
					message = ret.data.information;
	...
	var $this = $(this);
	$.post('/service/lottery/action/ajax_insert_record',$.param($('#ajax_form').serializeArray())+"&"+$.param({'prize_grade':prize_grade,'lottery_id':lottery_id}),function(ret){
		if(ret.code==0){
			alert(ret.message);
			}
		if(ret.code==1){
			alert('提交成功');
			window.location.href = '/service/lottery/action/lists';
		}
	},'json');
});
    </script>
